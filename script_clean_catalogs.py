...
df_init = pd.read_excel(
    ...
    dtype={"postal_code": str, "score": int}
).query(
...
regex_pattern = r'^[&,é,"\',\(\-è_ç,à]+\s+(rue|avenue|allée)'
correspondance = {
    '&': '1',
    'é': '2',
    '"': '3',
...
postal_code_in_street = df_demandeurs.apply(
    lambda x: (x["postal_code"] in x["street"]),
    axis = 1
)
df_demandeurs.loc[postal_code_in_street,"street"] = df_demandeurs[postal_code_in_street].apply(lambda x: x["street"].replace(x["postal_code"], ""), axis=1)

## Retirer ce qui n'est pas une lettre ou un chiffre au début
df_demandeurs['street'] = df_demandeurs['street'].replace(r'^[^\w\d-]*', '', regex=True)
...
lambda x: (x["street"] + x["address_1"]) if x["address_1"] != "" else (x["street"] + " " + x["address_2"]),
axis = 1 
)
...
df_demandeurs["address_1"] = df_demandeurs["address_1"].apply(lambda x: unidecode(x).upper()).str.strip()
df_demandeurs["address_2"] = df_demandeurs["address_2"].apply(lambda x: unidecode(x).upper()).str.strip()

# Supprimer les éléments des colonnes en doublons (si street et address_1 sont égales, on garde que street)
df_demandeurs["address_2"] = df_demandeurs.apply(
    lambda x: "" if (x["address_2"] == x["address_1"]) else x["address_2"],
    axis =1
)
df_demandeurs["address_1"] = df_demandeurs.apply(
    lambda x: "" if (x["street"] == x["address_1"]) else x["address_1"],
    axis =1
)

# Supprimer les demandeurs qui n'ont pas d'addresses
df_demandeurs = df_demandeurs[df_demandeurs.apply(lambda row: len(row['street'] + row['address_1'] + row['address_2']), axis=1) >= 3]
...
print("Export réussi. {} ({:.1%}) lignes ont été filtrées.".format(
    ...
    (df_init.shape[0] - df_conc.shape[0])/df_init.shape[0]
    )
)
